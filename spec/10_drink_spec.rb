...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      subject(:my_irn_bru) { described_class.new('irn bru') }

      it 'is your choice of beverage' do
        expect(my_irn_bru.type).to eq('irn bru')
      end

      it 'has 16 ounces' do
        expect(my_irn_bru.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      subject(:my_irn_bru) { described_class.new('irn bru', 20) }

      it 'is full' do
        expect(my_irn_bru).to be_full
      end
    end

    context 'when drink has less than 16 ounces' do
      subject(:my_irn_bru) { described_class.new('irn bru', 10) }

      it 'is not full' do
        expect(my_irn_bru).not_to be_full
      end
    end
  end
end
